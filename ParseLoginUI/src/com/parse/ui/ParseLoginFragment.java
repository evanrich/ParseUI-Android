...
public class ParseLoginFragment extends ParseLoginFragmentBase {
    ...
    private static final int GET_EMAIL_PASSWORD = 100;
    ...
    public void onActivityResult(final int requestCode, final int resultCode, final Intent data) {
        ...
        switch (requestCode) {
            ...
            case GET_EMAIL_PASSWORD:
                if (resultCode==Activity.RESULT_OK){
                    b = data.getExtras();

                    if (facebookUser != null && parseUser != null
                            && facebookUser.getName().length() > 0) {
                        parseUser.setUsername(b.getString("email"));
                        parseUser.setPassword(b.getString("password"));
                    }

                } else if (resultCode==Activity.RESULT_CANCELED) {
                    //User chose not to add username/password/name, so lets store the name from their Facebook account.
                    parseUser.put(USER_OBJECT_NAME_FIELD, facebookUser.getName());
                }

        }
        parseUser.put(USER_OBJECT_NAME_FIELD, facebookUser.getName());
        parseUser.put("fbId", facebookUser.getId()); //added
        parseUser.put("email", String.valueOf(facebookUser.getProperty("email"))); //added
    ...
    private void setUpFacebookLogin() {
        ...
        facebookLoginButton.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                //startActivityForResult(new Intent(getActivity(), EmailPassword.class), 1);
                loadingStart(true);
                        ...
                        if (user == null) {
                            ...
                            Request.newMeRequest(ParseFacebookUtils.getSession(),
                                ...
                                public void onCompleted(GraphUser fbUser,
                                     ...
                                     */
                                    //askForPassword(fbUser);
                                    facebookUser=fbUser;
                                    startActivityForResult(new Intent(getActivity(), EmailPassword.class), GET_EMAIL_PASSWORD);
